...
function actualizarBotonEdicion() {
  if (modoEdicionActivo) {
    btnToggleEdicion.innerHTML = "🔓 Desactivar edición";
    btnToggleEdicion.title = "Salir del modo edición";
  } else {
    btnToggleEdicion.innerHTML = "🔒 Activar edición";
...
btnToggleEdicion.addEventListener("click", (e) => {
  e.stopPropagation(); // evita que el click cierre mal el menú
  ...
  settingsMenu.classList.add("d-none"); // cerrar menú
});

function actualizarModoEdicion() {
  document.body.classList.toggle("modo-edicion", modoEdicionActivo);

  document.querySelectorAll(".boton-edicion-ui")
    .forEach(el => el.classList.toggle("d-none", !modoEdicionActivo));

  const icon = btnToggleEdicion.querySelector("i");
  icon.classList.toggle("text-warning", modoEdicionActivo);
}
...
const LS_VIP = "VIP_USERS";

function obtenerVIPs() {
  return JSON.parse(localStorage.getItem(LS_VIP)) || [];
}

function guardarVIPs(lista) {
  localStorage.setItem(LS_VIP, JSON.stringify(lista));
}
...
document.addEventListener("DOMContentLoaded", renderVIPs);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (rol === "vip") {
    settingsWrapper.style.display = "none"; // 👑 VIP no ve engranaje
  }
});
